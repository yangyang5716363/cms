...
const template = (data) => {
  ...
  if (data.length) {
    ...
    data.map((item, index) => {
      if (item.articleThumbnailUrl) {
        str +=  '<li class="clearfix">' + 
                  '<div class="flt newList-left">' +
                    '<img src="'+ item.articleThumbnailUrl +'" alt="" class="newList-left_cover">' +
                  '</div>'
      } else {
        str += '<li class="clearfix noCover">'
      }
        str +=  '<div class="frt newList-right">' +
                  ...
                  '<p class="iconbox">' +
                    ...
                    '<span id="count_' + item.id + '" class="count">' + item.clicks + '</span>' +
                  '</p>' +
                  '<div class="newDetail">' + item.articleAbstract + '</div>' +
                '<div>' +
              '</li>'
    })
  } else {
...
const paginationFun = (urlParams,jsonpUrl) => {
  $('#pagination-container').pagination({
    ...
    callback: (data, pagination) => {
      ...
      $('#content-html .newListBox li').click(function() {
...
const updateArticleClicks = (jsonpUrl, successFun, dom) => {
  let articleId = $(dom).find('.countClicks').attr('data_id')
  $.ajax({
    type: "get",
    async: false,
...
const updateArticleClicksSucc = (data, dom) => {
  const data_href =  $(dom).find('.countClicks').attr('data_href')
  const target = $(dom).find('.countClicks').attr('target') === '_blank' ? '_blank' : '_self'
